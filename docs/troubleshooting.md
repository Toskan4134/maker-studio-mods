...
3. **apiVersion is one this editor provides**. A mod asking for a version newer than the editor's (`"1.0.3"` on a 1.0.2 editor) or a different major (`"2.0.0"`) is rejected. Set it to the version that introduced the newest API you call — see [api-changelog.md](api-changelog.md).
